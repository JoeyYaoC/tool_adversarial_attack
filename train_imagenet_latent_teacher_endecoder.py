#!/usr/bin/env python3
"""Train a shared latent perturbation encoder and decoder from a teacher-bank dataset.

This script consumes the output of
``build_imagenet_adversarial_teacher_bank.py`` and trains:

- one shared ``LatentGridEncoder``
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a latent perturbation encoder/decoder from a teacher-bank dataset.",
    ...
    parser.add_argument("--lr-latent", type=float, default=8e-2, help="Learning rate for the encoder (formerly latent bank).")
...
def train_decoder(
    ...
    optimizer = torch.optim.Adam(
        [
            ...
            {"params": encoder.parameters(), "lr": lr_latent, "weight_decay": weight_decay},
        ]
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_index, batch in enumerate(loader, start=1):
            # latent_indices is kept for compatibility with Dataset output, but unused for the Encoder.
            latent_indices, images, teacher_advs, source_labels, target_labels, teacher_pred_labels = batch
            
            # Predict latents using the shared encoder instead of per-sample bank
