...
- a trained latent encoder/decoder checkpoint
- a classifier (LLR2 checkpoint or torchvision pretrained model)
- an ImageNet split from the LLR2 dataset setup

Then it leverages the encoder to provide an amortized initial guess for each 
selected image, optimizes that latent code, decodes that latent into a perturbation, 
and saves the resulting adversarial images plus a manifest/summary for later evaluation.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate adversarial ImageNet examples by refining amortized latents in a trained perturbation decoder.",
...
def load_latent_decoder_checkpoint(path: Path, device: torch.device) -> tuple[dict[str, Any], LatentGridDecoder, LatentGridEncoder]:
    ...
    if not isinstance(decoder_config, dict) or decoder_state_dict is None or encoder_state_dict is None:
        raise ValueError(f"{path} is missing decoder_config, decoder_state_dict, or encoder_state_dict")
...
def latent_attack(
    ...
    # Use the encoder for an amortized initial guess
    with torch.no_grad():
        init_latents = encoder(x0).detach()
        
    anchor = init_latents.clone().detach()
