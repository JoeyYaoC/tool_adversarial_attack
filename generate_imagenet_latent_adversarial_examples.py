...
class LatentGridDecoder(nn.Module):
    def __init__(self, grid_size: int, scale: float, latent_channels: int = 3, hidden_channels: int = 64) -> None:
        super().__init__()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target-mode",type=str,default="random",choices=("random", "least_likely", "fixed", "next_class"),help="How to choose target labels when --targeted is enabled.",)
...
def update_summary_stats(
    ...
    attack_stats["total_attack_time"] += batch_attack_time
...
def finalize_summary(summary: dict[str, Any], attack_name: str) -> dict[str, Any]:
    ...
    attack_stats["avg_attack_time_per_sample"] = (attack_stats["total_attack_time"] / attempted) if attempted else 0.0
...
def main() -> None:
    ...
    with manifest_path.open("w", encoding="utf-8") as manifest_file:
        for raw_batch in dataloader:
            ...
            batch_attack_time = time.time() - attack_start_time
            ...
            update_summary_stats(
                ...
                batch_attack_time=batch_attack_time,
            )
    ...
    LOGGER.info(
        "attack=%s attempted=%d success=%d success_rate=%.4f mean_l2=%.4f mean_linf=%.4f avg_time/sample=%.4fs", # <-- 这里加上了 avg_time/sample=%.4fs
        args.attack_name,
        ...
        attack_stats["avg_attack_time_per_sample"],
    )
